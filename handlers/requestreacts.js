    ...
    client.on("messageReactionAdd", async (reaction, user) => {
        ...
        switch (reactionemoji) {
          ...
          case String(emoji.react.show_queue):
            ...
            if (tracks.length < 15){
              user.send(embed.setDescription(tracks.map((track, i) => `**${++i})** [${track.title.substr(0, 35)}](${track.uri}) - ${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]} - **เพลงนี้ถูกขอโดยคุณ: ${track.requester.tag}**`).join("\n")))
              message.channel.send(new MessageEmbed()
                .setTitle(`${emoji.msg.SUCCESS} Check your direct messages to see the Queue`)
                .setColor(ee.color)
                .setFooter(ee.footertext, ee.footericon)
