      ...
      client.manager
        .on("playerCreate", async (player) => {
            ...
            var embed = new MessageEmbed();
              ...
              embed.setDescription(`**มีการเรียกใช้งานคำสั่งที่: ** <#${client.channels.cache.get(player.textChannel).id}>นี้นะคะไอพวกเวร`)
        ...
        .on("trackStart", async (player, track) => {
          try {
            ...
            var embed = new MessageEmbed().setColor(ee.color)
              ...
              embed.setFooter(`เพลงนี้ถูกขอโดยคุณ: ${track.requester.tag}`, track.requester.displayAvatarURL({dynamic: true}));
            var irc = await isrequestchannel(client, player.textChannel, player.guild);
            if(irc) {
                    ...
                    switch (reactionemoji) {
                      case String(emoji.react.rewind):
                        ...
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.rewind} กรอเพลงกลับไป : 20 วินาที่, to: ${format(Number(player.position))}`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable)
                      ...
                      case String(emoji.react.forward):
                        ...
                        //send an information message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.forward} กรอเพลงไป : 20 วินาที่, to: ${format(Number(player.position))}`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable)
                      ...
                      case String(emoji.react.pause_resume):
                        ...
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${player.playing ? `${emoji.msg.resume} เล่น` : `${emoji.msg.pause} หยุดเล่น`} เพลงต่อ.`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                      ...
                      case String(emoji.react.stop):
                        ...
                        //send information message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.stop} เอาและเราจะหยุดเล่นเพลงและออกจากห้องของคุณและนะชิ!!!`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.previous_track):
                        ...
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.previous_track} เราจะกลับไปเล่นเพลงก่อนหน้านี้และนะชิสั่งอยู่ได้`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.skip_track):
                        ...
                        if (client.settings.get(message.guild.id, `djroles`).toString() !== "") {
                          ...
                          if (player.queue.size == 0) {
                            ...
                            return message.channel.send(new MessageEmbed()
                              .setTitle(`${emoji.msg.stop} ไม่มีคิวเพลงถัดไปแล้วงั้นเราออกและนะ`)
                              .setColor(ee.color)
                              .setFooter(ee.footertext, ee.footericon)
                            );
                          }
                          ...
                          //send success message
                          return message.channel.send(new MessageEmbed()
                            .setTitle(`${emoji.msg.skip_track} ข้ามไปเพลงถัดไปและนะฮึ!!!`)
                            .setColor(ee.color)
                            .setFooter(ee.footertext, ee.footericon)
                          );
                        }
                      ...
                      case String(emoji.react.replay_track):
                        ...
                        //send an information message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.replay_track} งืมมอยากฟังเพลงนี้ซ้ํานะเหรอเราเข้าใจแล้วและ`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.reduce_volume):
                        ...
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.reduce_volume} ดังเกินละสิรู้นะลดเสียงให้ก็ได้สัก : **${player.volume} %**`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.raise_volume):
                        ...
                        //send an informational message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.raise_volume} เสียงเบาเกินงั้นเหรอเข้าใจแล้วๆเพิ่มเสียงเป็น : **${player.volume} %**`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.toggle_mute):
                        ...
                        //send an informational message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${player.volume === 0 ? `${emoji.msg.toggle_mute} มิลเสียงให้และนะ` : `${emoji.msg.reduce_volume} เอาและๆเปิดเสียงให้เหมือนเดิมก็ได้เรื่องมากจริง`}`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.repeat_mode):
                        ...
                        if (!player.trackRepeat && !hasmap.get(message.guild.id)) {
                          ...
                          //Send an informational message
                          message.channel.send(new MessageEmbed()
                            .setTitle(`${emoji.msg.repeat_mode} เปิดลูปเลยนะ ${player.trackRepeat ? `${emoji.msg.enabled} ` : `${emoji.msg.disabled} `}.`)
                            .setDescription(`ลูปคิวเล่นเพลง ${player.queueRepeat ? `${emoji.msg.enabled} Enabled` : `${emoji.msg.disabled} Disabled`}.`)
                            .setColor(ee.color)
                            .setFooter(ee.footertext, ee.footericon)
                          ).then(msg => {
                            if(msg && msg.deletable)
                        ...
                        //if track repeat mode is on and queue repeat mode off
                          ...
                          message.channel.send(new MessageEmbed()
                            .setTitle(`${emoji.msg.repeat_mode} ลูปคำขอเพลง ${player.queueRepeat ? `${emoji.msg.enabled} ` : `${emoji.msg.disabled} `}.`)
                            .setDescription(`และเทร็กลูป ${player.trackRepeat ? `${emoji.msg.enabled} ` : `${emoji.msg.disabled} `}.`)
                            .setColor(ee.color)
                            .setFooter(ee.footertext, ee.footericon)
                          ).then(msg => {
                            if(msg && msg.deletable) msg.delete({
                        ...
                        //otherwise like queue on and track should be off...
                          ...
                          //send informational message
                          message.channel.send(new MessageEmbed()
                            .setTitle(`${emoji.msg.repeat_mode} ลูปคำขอเพลงเดียวนี้ ${player.queueRepeat ? `${emoji.msg.enabled} ` : `${emoji.msg.disabled} `}.`)
                            .setDescription(`เปิดลูปเทร็ก ${player.trackRepeat ? `${emoji.msg.enabled} ` : `${emoji.msg.disabled} `}.`)
                            .setColor(ee.color)
                            .setFooter(ee.footertext, ee.footericon)
                          ).then(msg => {
                            if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.autoplay_mode):
                        ...
                        //Send Success Message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${player.get("autoplay") ? `${emoji.msg.enabled} เปิดใช้งาน` : `${emoji.msg.disabled} ปิดใช้งาน`} ระบบเล่นเพลงอัตโนมัติและนะ`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.shuffle):
                        ...
                        //send informational message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`${emoji.msg.shuffle} เอะะะจะสลับคิวเพลงอย่างนั้นเหรอ.`)
                          .setColor(ee.color)
                          .setFooter(ee.footertext, ee.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
                      ...
                      case String(emoji.react.show_queue):
                        //define the Embed
                        var embed = new MessageEmbed()
                          .setAuthor(`คำขอเพลงทั้งหมดมีตามนี้อะนะ ${message.guild.name}  -  [ ${player.queue.length} Tracks ]`, message.guild.iconURL({
                            dynamic: true
                          }))
                          .setColor(ee.color);
                        ...
                        if (tracks.length < 15)
                          return message.channel.send(embed.setDescription(tracks.map((track, i) => `**${++i})** [${track.title.substr(0, 35)}](${track.uri}) - ${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]} - **เพลงนี้ถูกขอโดยคุณ: ${track.requester.tag}**`).join("\n"))).then(msg => {
                            if(msg && msg.deletable) msg.delete({
                                timeout: 4000
                              }).catch(e => console.log("couldn't delete message this is a catch to prevent a crash".grey));
                          })
                        ...
                        for (var i = 0; i < tracks.length; i += 15) {
                          ...
                          quelist.push(songs.map((track, index) => `**${i + ++index})** [${track.title.split("[").join("{").split("]").join("}").substr(0, 35)}](${track.uri}) - ${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]} - **เพลงนี้ถูกขอโดยคุณ: ${track.requester.tag}**`).join("\n"))
                        }
                      ...
                      case String(emoji.react.show_current_track):
                        ...
                        return message.channel.send(new MessageEmbed()
                          .setAuthor("เพลงที่กำลังเล่นในขณะนี้:", user.displayAvatarURL({
                            dynamic: true
                          }))
                          ...
                          .setFooter(`เพลงนี้ถูกขอโดยคุณ: ${player.queue.current.requester.tag}`, player.queue.current.requester.displayAvatarURL({
                            dynamic: true
                          }))
                        ).then(msg => {
                          if(msg && msg.deletable)   msg.delete({
